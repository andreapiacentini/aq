...
module aq_change_var_mod

use kinds
use aq_convert_q_to_x_mod
use aq_convert_x_to_q_mod
use aq_convert_x_to_u_mod
use aq_convert_x_to_v_mod
use aq_fields_mod
...
private
public :: aq_change_var_registry
public :: aq_change_var
public :: aq_change_var_tl,aq_change_var_ad
! ------------------------------------------------------------------------------
type :: aq_change_var_config
  ...
  logical :: x_to_u
  logical :: x_to_v
end type aq_change_var_config

#define LISTED_TYPE aq_change_var_config

!> Linked list interface - defines registry_t type
#include "oops/util/linkedList_i.f"

!> Global registry
type(registry_t) :: aq_change_var_registry
! ------------------------------------------------------------------------------
contains
! ------------------------------------------------------------------------------
! Public
...
subroutine aq_change_var_setup(conf,fld_in,fld_out,ad)

implicit none

! Passed variables
type(aq_change_var_config),intent(inout) :: conf !< Variable change
type(aq_fields),intent(in) :: fld_in             !< Input field
type(aq_fields),intent(in) :: fld_out            !< Output field
...
if (conf%x_out.and.(.not.(conf%x_in.or.conf%q_in))) call abor1_ftn('aq_change_var_setup: x or q required to compute x')
if (conf%q_out.and.(.not.(conf%x_in.or.conf%q_in))) call abor1_ftn('aq_change_var_setup: x or q required to compute q')
if (conf%u_out.and.(.not.(conf%x_in.or.conf%q_in.or.conf%u_in))) &
 & call abor1_ftn('aq_change_var_setup: x, q or u required to compute u')
if (conf%v_out.and.(.not.(conf%x_in.or.conf%q_in.or.conf%v_in))) &
 & call abor1_ftn('aq_change_var_setup: x, q or v required to compute v')
...
end subroutine aq_change_var_setup
! ------------------------------------------------------------------------------
!> Get variables
subroutine aq_change_var_get(conf,fld,x,q,u,v)

implicit none

! Passed variables
type(aq_change_var_config),intent(in) :: conf                         !< Variable change
...
end subroutine aq_change_var_get
! ------------------------------------------------------------------------------
!> Set variables
subroutine aq_change_var_set(conf,fld,x,q,u,v)
...
end subroutine aq_change_var_set
! ------------------------------------------------------------------------------
!> Change of variable
subroutine aq_change_var(fld_in,fld_out)

implicit none

! Passed variables
type(aq_fields),intent(in) :: fld_in     !< Input fields
type(aq_fields),intent(inout) :: fld_out !< Output fields
...
type(aq_change_var_config) :: conf

! Check resolution
call aq_fields_check_resolution(fld_in,fld_out)

! Copy boundary conditions
call aq_fields_copy_lbc(fld_out,fld_in)

! Define change of variable configuration
call aq_change_var_setup(conf,fld_in,fld_out)

! Get x, q, u and v
call aq_change_var_get(conf,fld_in,x,q,u,v)
...
! Set x, q, u and v
call aq_change_var_set(conf,fld_out,x,q,u,v)

end subroutine aq_change_var
! ------------------------------------------------------------------------------
!> Change of variable
subroutine aq_change_var_tl(fld_in,fld_out)
...
end subroutine aq_change_var_tl
! ------------------------------------------------------------------------------
!> Change of variable - adjoint
subroutine aq_change_var_ad(fld_in,fld_out)
...
! Define change of variable configuration
call aq_change_var_setup(conf,fld_in,fld_out,.true.)
...
end subroutine aq_change_var_ad
! ------------------------------------------------------------------------------
end module aq_change_var_mod
