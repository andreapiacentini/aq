...
namespace aq {
// -----------------------------------------------------------------------------
GeometryAQ::GeometryAQ(const GeometryAqParameters & params,
...
// -----------------------------------------------------------------------------
GeometryAQ::GeometryAQ(const GeometryAQ & other) : comm_(other.comm_) {
  // Copy ATLAS grid
...
// -----------------------------------------------------------------------------
GeometryAQ::~GeometryAQ() {
  aq_geom_delete_f90(keyGeom_);
}
// -----------------------------------------------------------------------------
GeometryAQIterator GeometryAQ::begin() const {
  return GeometryAQIterator(*this);
}
// -----------------------------------------------------------------------------
GeometryAQIterator GeometryAQ::end() const {
  int nx = 0;
  int ny = 0;
  int nz;
  double deltax;
  ...
  return GeometryAQIterator(*this, nx*ny+1);
}
// -------------------------------------------------------------------------------------------------
std::vector<double> GeometryAQ::verticalCoord(std::string & vcUnits) const {
...
// -------------------------------------------------------------------------------------------------
std::vector<size_t> GeometryAQ::variableSizes(const oops::Variables & vars) const {
...
// -----------------------------------------------------------------------------
void GeometryAQ::print(std::ostream & os) const {
  int nx;
  int ny;
  int nz;
  double deltax;
